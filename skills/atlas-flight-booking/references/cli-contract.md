...
On `AUTHORIZED`, retain `data.ticketing_available` and the optional `data.ticketing_activation_url` and `data.ticketing_blocker` for the current conversation. Show the URL only when it was returned and ticketing is unavailable. `TICKETING_ACTIVATION_REQUIRED` means the remaining ticketing activation steps are incomplete; do not guess which step. `TOP_UP_REQUIRED` means real-time flight and price search remains available, but price verification, order creation, and ticketing require a completed balance top-up. If the user completes the indicated steps, check authorization status again and run a new search; never continue an earlier non-bookable offer.
